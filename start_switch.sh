...
tmux send-keys -t 1 "./run_switchd.sh -p $P4_PROG --arch tf2" C-m

mkdir -p $SDE/logs

tmux send-keys -t 2 "cd $SDE" C-m
tmux send-keys -t 2 "./run_tofino_model.sh -p $P4_PROG --arch tf2 -f $P4JOIN_SRC/p4src/ports.json --log-dir $SDE/logs" C-m
